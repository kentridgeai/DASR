...
def kexp_to_tree(kexp, ordered_symbols = True):
    ...
    for symbol in kexp:
        ...
        elif symbol in ("R","scr/a","age","(xdata[3])","(xdata[4])","sys/c","(xdata[5])","(xdata[6])","weight","height"):
            pass
    if ordered_symbols:
        queue = [root]  # FIFO queue
        all_nodes = [root]
        ...
        for node in all_nodes:
            if node.symbol not in ("R","scr/a","age","(xdata[3])","(xdata[4])","sys/c","(xdata[5])","(xdata[6])","weight","height") and operators[node.symbol]["symmetric"]:
